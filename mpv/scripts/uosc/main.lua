...
function update_duration()
	local duration = state._duration
		and (
			(state.rebase_start_time == false and state.start_time) and (state._duration + state.start_time)
			or state._duration
		)
...
bind_command(
	...
	create_select_tracklist_type_menu_opener({
		...
		type = "audio",
		prop = "aid",
		load_command = "script-binding uosc/load-audio",
	})
)
bind_command(
	"video",
	create_select_tracklist_type_menu_opener({
		...
		type = "video",
		prop = "vid",
		load_command = "script-binding uosc/load-video",
	})
)
bind_command(
	"playlist",
	create_self_updating_menu_opener({
		...
		on_paste = function(event)
			mp.commandv("loadfile", tostring(event.value), "append")
		end,
...
bind_command(
	...
	create_self_updating_menu_opener({
		...
		on_activate = function(event)
			mp.commandv("set", "edition", event.value)
		end,
	})
)
...
bind_command("menu-prev", function()
	Elements:maybe("menu", "navigate_by_items", -1)
end)
bind_command("menu-next", function()
	Elements:maybe("menu", "navigate_by_items", 1)
end)
bind_command("menu-prev-page", function()
	Elements:maybe("menu", "navigate_by_page", -1)
end)
bind_command("menu-next-page", function()
	Elements:maybe("menu", "navigate_by_page", 1)
end)
bind_command("menu-start", function()
	Elements:maybe("menu", "navigate_by_items", -math.huge)
end)
bind_command("menu-end", function()
	Elements:maybe("menu", "navigate_by_items", math.huge)
end)
bind_command("menu-activate", function()
	Elements:maybe("menu", "activate_selected_item")
end)
bind_command("menu-back", function()
	Elements:maybe("menu", "back")
end)
bind_command(
	"audio-device",
	create_self_updating_menu_opener({
		...
		on_activate = function(event)
			mp.commandv("set", "audio-device", event.value)
		end,
	})
)
